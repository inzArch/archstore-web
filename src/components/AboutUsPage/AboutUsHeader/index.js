...
const Logo_Blue =
	'https://archstore-website.s3.ap-south-1.amazonaws.com/static/media/Logo_Blue.png';
const AboutUsHeader = () => {
	return (
					...
					>
						<Nav className='me-auto'>
							{AboutUsHeaderContent.map((option, i) => (
								<NavDropdown
									title={option.name}
									id='navbarScrollingDropdown-expand'
								>
									{option.content.map((content, i) => (
										<>
											{Object.keys(content).includes('category') ? (
												<NavDropdown title={content.item}>
